...
Both methods have to start with `08-prepareClustering.R`to prepare the clustering. One has to comment, or uncomment, the method they want to use, or not use, at the beginning of the script. \\
...
It has to contains the divergence time, although unprecisions are no big deal. The important is to have a correct topology and no polytomy 
